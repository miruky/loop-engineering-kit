...
Mutable state lives under `.agentkit/state/`; derived output under `.agentkit/output/`. Both are ignored by Git. Atomic writes prevent a half-written JSON record. An exclusive local writer lock prevents overlapping controllers in one project. This is a single-machine tool, not a distributed scheduler or an NFS locking protocol.
